  ...
  const filteredGames = useMemo(() => {
    let filtered = mockGames.filter(game => {
      const matchesSearch = 
        ...
        game.locationText.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      const matchesSkill = 
        selectedSkillLevel === "all" || game.skillLevel === selectedSkillLevel;

      return matchesSearch && matchesSport && matchesSkill && game.status !== 'cancelled';
    });
    ...
    filtered.sort((a, b) => {
      if (sortBy === "date") {
        ...
        const aSpotsLeft = a.neededPlayerCount - a.currentPlayerCount;
        const bSpotsLeft = b.neededPlayerCount - b.currentPlayerCount;
        return bSpotsLeft - aSpotsLeft;
      }
    });

    return filtered;
  }, [searchTerm, selectedSports, selectedSkillLevel, sortBy]);
